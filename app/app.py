...
# ── HEART DISEASE ────────────────────────────────────────────────────
    ...
    # --------------------------
    # Monkey‑patch all OneHot encoders inside heart_pre
    for name, trans, cols in heart_pre.transformers_:
        if hasattr(trans, "categories_"):
            # strip out any NaN from each categories_ array
            clean = []
            for cat_list in trans.categories_:
                clean.append([c for c in cat_list if not (isinstance(c, float) and np.isnan(c))])
            trans.categories_ = clean
    # --------------------------

    # Pad & reorder to all expected columns
